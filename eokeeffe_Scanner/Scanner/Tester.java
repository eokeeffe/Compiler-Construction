...
public class Tester
{
	...
	/**
	 * Tests the compiler implementation with the given sourceFile. The file
	 * with the expected output is assumed to to have the name sourceFile +
	 * ".exp".
	 */
	private static boolean isCompilerCorrect(File sourceFile,
			boolean generateOutputs)
	{
		...
		try
		{
			...
			if (generateOutputs)
			{
				PrintStream out = new PrintStream(new File(sourceFilename
						+ ".out"));
				out.println(result);
			}
		}
	...
	public static void main(String[] args)
	{
		...
		{
			System.out
					.println("Usage: java Tester (<testFileDirectory> | <testFile1> <testFile2> ...)");
			System.exit(1);
		}
		...
		for (String sourceFilename : args)
		{
			...
			out.print("Test " + sourceFile.getName() + " ");
			if (isCompilerCorrect(sourceFile, true))
			{
				out.println("PASSED");
				score++;
			}
